...
class DesktopBackend(Protocol):
    ...
    def scroll(self, window: DesktopWindow, point: PixelPoint, amount: int) -> None: ...
    def paste_and_submit(self, window: DesktopWindow, point: PixelPoint, text: str) -> None: ...
```
...
2. `CodexAgent` converts the semantic action and fractional coordinates into a
   desktop operation that carries the exact target window identity.
3. `WindowsDesktop` activates the exact target window.
4. Only after successful activation, and another exact-window check at each
   global mouse or keyboard injection boundary, does it inject input or invoke
